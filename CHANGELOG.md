...
- **Issue #147 prove (CLI-default + software_dev as_tool/handoff):** `scripts/prove_issue147.py` prints PASS/FAIL for the programmatic path — `/v1/cli-agents` / `os-cli cli-agents --json` start set is PATH-discovered host CLIs only (no fake/echo CLIs), and `software_dev` CoS + engineer + skeptic wire **both** `as_tool` and `handoff`. Live `/v1/cli-agents` and pi skip when the server/binary is missing. `SWARM_TEST_MODE`, no Neon, no secrets. Fixes #147.
...
- **WebUI context-window usage per seat (Issue #215):** Composer badge + seat-settings detail show estimated tokens of current model context (messages + spliced summaries + system/instructions + tool-schema overhead) against the seat's declared window. Unknown windows stay honest (`~12.3k tokens, window unknown`). Estimate is chars/4 until a per-provider tokenizer is wired. `GET /chat/context-usage/` plus a WS `context_usage` event after each turn; compact / include-in-context toggle return the same payload so unticked summaries drop the count. Optional sparkline of growth over the session. No runtime engine changes. Fixes #215.
- **CLI-first default modes (Issues #151 / #149):** Fresh/default config turns CLI on and API / Blueprint / Team / Remote off until Settings → Rail enables them. Disabled modes stay out of the default rail/navbar (Manage CLI/API/Team/Remote footers stay consistent with #109). Rail and the CLI picker start from discovered host CLIs only — `known` is the catalog, `discovered`/`installed` is the PATH seed, `configured` is opt-in; pi absent stays absent. `build_starter_config` / `GET /v1/cli-agents/` advertise `settings.product_modes` + `modes` / `mode_limitations`. Tests: `tests/core/test_cli_first_defaults.py`. No LiteLLM catalog / no Neon / no secrets. Fixes #151. Fixes #149.
- **CoS first-class agent doctrine (Issue #219):** API Chief of Staff keeps persistent `create_agent` (REQ-154) — not TrueForge-style temp sub-agents — and now gets section/topology tools: `create_section` / `rename_section` / `archive_section` / `move_agent_to_section` / `set_talk_acl` / `list_sections`. Store is `agent_sections.json`. Talk ACL writes `agent_mailbox_acl` (new `section` entry kind; XOR allow/deny). Only CoS (API-kind) sees topology tools; Support stays lifecycle-only; ordinary roles never get them. Unknown agent/section ids fail honestly. Wired through `ChiefOfStaffRole.attach_as_tool`. Docs: [COS_TOPOLOGY.md](docs/COS_TOPOLOGY.md). Fixes #219.
- **Remote CLI connections (Issue #180):** Catalog flags `opencode` and `kilocode` as headless **serve** capable (`opencode serve` / `kilo serve`). Add-agent CLI wizard shows an optional Remote connection step (host, port, auth env, fleet box) only for those CLIs. Per-agent endpoint is stored on the rail seat; `cli_remote_boxes` registers several boxes; chat/run `params.cli_remote` picks per-session. Unset falls back to local. Sessions opened against a remote CLI are labeled `on host:port`. Auth is an env-var name — never a plaintext password. Fixes #180.
- **Team composer Roles pane (Issue #104):** Add agents first (roster cards are name / kind / remove only). A Roles drop zone unlocks once the roster is non-empty; drag or Add canonical roles (`support`, `gate`, `skeptic`, `chief_of_staff`, `suggestions`, `engineer`) to create slots, then assign unroled members. Distinct drag MIME so agents and roles cannot cross drop zones. One agent, one non-default role; CoS stays optional and API/CLI-only. Persist path unchanged (`members[].role`, `chief_of_staff_id`). Does not implement #105 / #106 / #107. Fixes #104.
- **Section internal-only talk lock (Issue #163):** Custom rail sections get a lock control (**Talk internal only** / **Talk externally**). Locked members may `list_agents` / `send_message` only to each other, including a team of one; unlock restores normal reach. Flag persists on `swarm_rail_sections`; chat turns snapshot it as `params.rail_sections` so mailbox tools enforce the same rule (no ACL dual-write). Unassigned is never lockable. Tests: `tests/core/test_section_talk.py`, `railSections.test.ts`. [docs/qa/ISSUE-163-section-internal-talk.md](docs/qa/ISSUE-163-section-internal-talk.md). Fixes #163.
- **REQ-887 Teams + Plugins marketplaces (#292):** In-app catalog reuses the Official MCP Registry (cached, last-good on offline/rate-limit) for plugins, Agent Skills (`SKILL.md` / `parse_skill_md`) for skill packs, and OS-native `team_rosters` packs (`swarm-team-pack`) for teams — no CrewAI/A2A-as-team-pack fiction, no second MCP index. `GET /v1/marketplace/catalog|preview` + `POST /v1/marketplace/install/` write into the existing plugin list, user `skills/` dir (copy only), and `/v1/team-rosters/`. GitHub topic scan stays the unpublished-plugin / team-pack fallback. Listings are env **names** only. Docs: [MARKETPLACE.md](docs/MARKETPLACE.md). Tests: `test_mcp_registry.py`, `test_marketplace_catalog.py`, `installCatalog.test.ts`. Fixes #292.
- **REQ-858 Plugins/tools & skills install UX (#213):** PluginsPopup gains **Add tools** / **Add skills** catalog panes (search, kind filters, cards, detail drawer, one-click install with progress + per-server connect check, already-installed Manage/Remove, health dot). GitHub marketplace scan feeds community cards; scan payloads without a command stay non-installable (honest hint). Skills is an empty state on purpose until a skill-pack catalog lands. Env **names** only — never values. DaisyUI house style. Tests: `InstallCatalog.test.tsx`, `installCatalog.test.ts`, `test_req858_plugins_install_ux.py`. Fixes #213.
- **Public demo site with mocked inference (REQ-882 / #279):** `SWARM_DEMO_MODE=1` intercepts chat turns with a scripted engine (SDLC handoff, CLI, Hermes/TrueForge remote, team CoS) — no paid LLM, no local shells. Static SPA builds with `VITE_DEMO_MODE=true` (`npm run build:demo`) and a client-side mock WebSocket/REST layer. Composer steerage: scenario banner, suggestion chips, guided tour, fallback notice for arbitrary prompts. Operating Swarm branding unchanged. Fly.io (`fly.demo.toml`, `open-swarm-demo`) and Pages deploys are operator-gated via `scripts/deploy_demo_site.py` (skip-0 when credentials are missing). Tests: `test_req882_*`, `test_demo_script_engine.py`, `test_consumer_demo_mode.py`, Vitest `lib/demo/*`. Fixes #279.
- **REQ-883 desktop packaging plan (Windows + macOS):** [docs/qa/REQ-883-desktop-app-packaging-windows-macos.md](docs/qa/REQ-883-desktop-app-packaging-windows-macos.md) extends ADR-003 to a self-contained **Operating Swarm** desktop app (pywebview + PyInstaller onedir; Electron rejected). Scaffolds `swarm-desktop --print-plan` (IPv4 loopback, user-profile sqlite, first-run `token_urlsafe` secret, PATH merge for host CLIs). Window / freeze / NSIS / `.dmg` / signing are REQ-883B–F. Source lock: `tests/unit/test_req883_desktop_app_packaging.py`. Refs #280.

- **Open WebUI remote (Issue #90):** Opt-in external remote (`openwebui`) lists chats as searchable sessions, resumes a selected chat, and streams `/api/chat/completions` (sync fallback) into the OS WebUI transcript. Not Operating Swarm's own WebUI (`os-webui`) and does not replace it. Env: `OPENWEBUI_BASE_URL` / `OPENWEBUI_API_KEY` (placeholders only). Tests: `tests/core/test_openwebui_remote.py`. Fixes #90.
...
- **`software_dev` remote workdir (Issue #148, partial):** CoS / engineer / skeptic file tools stay API-host local by default (`status` states FS-locality). `params.remote_workdir` or a remote-shaped `params.workdir` (`user@host:path` / `ssh://user@host/path`, or `ssh_host`+`ssh_user` plus a path) hops via OpenSSH using the Herdr argv builder. Each remote argv element is shell-quoted so OpenSSH space-join + login-shell parse keeps a multiline `python3 -c` helper intact (list-only stubs hid this). Identity is an env-var name for a key path — never a private key. Local `..` / prefix-sibling escapes still refused. Success(1) is not claimed Met (quoted hop + join regression is not a live SSH hop). Live dev-worker-max `:8002` → dev-worker-gpu prove remains. Refs #148. OpenSSH argv fleet note: Refs #157.

### Fixed
- **Test-schedule `update_schedule` now persists `next_run` (REQ-896):** the patch key was whitelisted but never applied, so schedules could not be seeded/corrected and `tick_due_schedules` never fired them.
- **CLI list-models probes report honest failures (REQ-896 / #272):** timeout, auth failure, and empty stdout return `models: []` (or last-good from cache) instead of fabricated catalog presets; presets remain only for a CLI that is not installed, and a missing-CLI probe can no longer evict last-good from the cache.
- **PWA brand files are one truth (REQ-896):** `assets/brand/` canonical `manifest.json` / `favicon-minimal.svg` / `webui-geometric.svg` re-synced with the SPA copies so the served manifest carries the Operating Swarm identity (REQ-862) instead of the stale "Open Swarm".
- **REQ-877 CLI model probes no longer stall `/v1/llm-profiles/`:** Profile loading probes configured CLIs concurrently (1.5s cap), caches results for 10 minutes, and serves last-good / stale models immediately on timeout, auth failure, or TTL expiry so WebUI reloads never block on a hanging `claude`/`gemini`/… subprocess. Fixes #272.
- **Remote operate replies are the remote's text (Issues #300–#305):** OpenMousBot list uses slim `GET /api/bots?messages=0` (no transcripts). OMB/Hermes send poll after 202/`POST /v1/runs` until the assistant text exists; timeout is a named error, never a UUID/run ACK. SPA send abort is 180s (list stays 12s). Herdr operate send waits `--until idle` and returns pane text like `chat_herdr`. AnythingLLM Settings list fills the picker from `sessions` (`workspace:thread`) and send forwards that id. Refs #300 #301 #302 #303 #304 #305.
- **Plugins popup freeze order (REQ-881 / #278):** Toggling a tool On/Off in the Plugins overlay no longer re-sorts the list under the cursor. Enabled-first order is snapshotted when the popup opens (or the catalog loads); search still filters in that frozen order. Reopen to re-sort. Tests: `PluginsPopup.test.tsx`, `chatPluginTools.test.ts`. Fixes #278.
...
- **Demo SDLC BA / `sdlc_handoff` blueprint REPL calls the saved LLM profile:** Talking to **ba** on the demo SDLC team no longer echoes `[team:… target:ba] {user}`. Roster members with `source: blueprint:sdlc_handoff` run the blueprint, which calls `chat.completions` with the persisted profile (`base_url` + `model`). Unreachable LLM returns an honest error — never a silent echo. Named profiles round-trip via POST/PUT + GET `/v1/llm-profiles/` (WebUI shows model and base URL after reload). Closes #113.
