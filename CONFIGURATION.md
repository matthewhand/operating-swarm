...
- **Per-task override (REQ-43):** `settings.override_per_task` plus `settings.task_llm_profiles` map task classes (`orchestration` / `auxiliary` / `delegation`, plus the #858/#859/#860 slots `tiny` / `compaction` / `autocomplete`) to any connected model id. Those names are roles, not required slugs. SPA Settings auto-picks three models when the user never opens the picker. Live CLI catalogs come from sibling REQ-44 (`{cli, models}`) when that helper is merged; until then auto-pick stubs on `/v1/models` + fixtures and does not scrape CLI `--help`. Off = everything uses Default.
- **Session labelling (#731):** session retitling/labels are a lightweight background summarisation task routed through the `tiny` override, then `auxiliary`, then the API default — list session labelling alongside code summary as an auxiliary responsibility.
- **Fallbacks:**
...
| `DJANGO_CSRF_TRUSTED_ORIGINS` | Comma-separated trusted origins for CSRF on mutating routes (whitespace-trimmed, empties dropped). Must include scheme + host + port. Add LAN/proxy origins (e.g. `http://198.51.100.30:8000`) when the UI is not on localhost. | `http://localhost:8000,http://127.0.0.1:8000` |
