...
describe('AgentAvatar', () => {
  ...
  it('uses the factory default theme when no custom src is set', () => {
  ...
  it('uses the bland two-dot glance when opted in via settings', () => {
    saveAvatarTheme('bland')
    const { container } = render(<AgentAvatar agentId="codey" />)
    const face = container.querySelector('[data-agent-avatar]')
    expect(face).toHaveAttribute('data-agent-avatar', 'default')
    expect(face).toHaveAttribute('data-avatar-theme', 'bland')
    expect(face).toHaveAttribute('data-eye-state', 'idle')
    const svg = container.querySelector('svg[data-avatar-theme="bland"]')
    expect(svg).toBeInTheDocument()
    expect(svg).toHaveAttribute('data-eye-state', 'idle')
    expect(svg?.querySelectorAll('.os-bland-eyes circle')).toHaveLength(2)
    expect(container.querySelector('img')).not.toBeInTheDocument()
  })

  it('sets bland eye state active when the agent is working', () => {
    saveAvatarTheme('bland')
    const idle = render(<AgentAvatar agentId="codey" active={false} />)
    expect(idle.container.querySelector('svg[data-avatar-theme="bland"]')).toHaveAttribute(
      'data-eye-state',
      'idle',
    )
    idle.unmount()
    const active = render(<AgentAvatar agentId="codey" active />)
    expect(active.container.querySelector('svg[data-avatar-theme="bland"]')).toHaveAttribute(
      'data-eye-state',
      'active',
    )
    active.unmount()
  ...
  it('treats blank custom src as the default theme face', () => {
  ...
  it('falls back to the default theme face when a custom src errors', () => {
  ...
  it('falls back to bland two-dot glance when a custom src errors under bland theme', () => {
    saveAvatarTheme('bland')
    const { container } = render(<AgentAvatar src="/avatars/missing.png" agentId="codey" />)
    const img = container.querySelector('img')!
    expect(img).toHaveAttribute('src', '/avatars/missing.png')
    ...
    expect(container.querySelector('svg[data-avatar-theme="bland"]')).toHaveAttribute(
      'data-eye-state',
      'idle',
    )
  })
  ...
  it('caps xs so it cannot fill an unbounded flex parent (#736)', () => {
    ...
    expect(css).toMatch(/\.os-bland-avatar--xs\s*\{[^}]*max-width:\s*1\.5rem/)
  ...
  it('uses a generated still on Bland and ignores it while Bee is selected', () => {
    rememberGeneratedAvatar('codey', '/avatars/codey_still.png')
    saveAvatarTheme('bee')
    const bee = render(<AgentAvatar agentId="codey" />)
    expect(bee.container.querySelector('[data-avatar-theme="bee"]')).toBeInTheDocument()
    expect(bee.container.querySelector('[data-avatar-still="generated"]')).not.toBeInTheDocument()
    bee.unmount()
  ...
  it('renders the 3D robot theme with a non-blocking SVG fallback in jsdom', () => {
    ...
    expect(container.querySelector('[data-robot3d-static]')).toHaveAttribute('data-eye-state', 'idle')
  })

  it('sets robot3d fallback eyes active when the agent is working', () => {
    saveAvatarTheme('robot3d')
    const { container } = render(<AgentAvatar agentId="codey" active />)
    expect(container.querySelector('[data-robot3d-static]')).toHaveAttribute(
      'data-eye-state',
      'active',
    )
    expect(container.querySelector('[data-robot3d-mode]')).toHaveAttribute(
      'data-eye-state',
      'active',
    )
  })

  it('disables bland/robot/blob/bee eye loops under reduced motion', () => {
    const css = fs.readFileSync(path.resolve(__dirname, '../../index.css'), 'utf8')
    expect(css).toMatch(
      /prefers-reduced-motion:\s*reduce[\s\S]*?\.os-bland-avatar\[data-eye-state="active"\] \.os-bland-eyes/,
    )
    expect(css).toMatch(
      /prefers-reduced-motion:\s*reduce[\s\S]*?\.os-robot-avatar\[data-eye-state="active"\] \.os-robot-pupils/,
    )
    expect(css).toMatch(
      /prefers-reduced-motion:\s*reduce[\s\S]*?\.os-robot3d-fallback\[data-eye-state="active"\] \.os-robot3d-pupils/,
    )
    expect(css).toMatch(/@keyframes os-bland-wander/)
    expect(css).toMatch(/@keyframes os-robot-wander/)
    expect(css).toMatch(/@keyframes os-robot3d-wander/)
  })
