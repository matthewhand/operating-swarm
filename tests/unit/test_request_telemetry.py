...
def test_record_and_summary_counts_recent_paths():
    ...
    summary = telemetry.client_summary("10.0.0.5", now=now)
...
def test_window_expiry_drops_old_records():
    telemetry = RequestTelemetry(window_seconds=60, max_records_per_client=100)
    now = time.monotonic()
    _record(telemetry, "10.0.0.5", "/v1/old/")
    summary = telemetry.client_summary("10.0.0.5", now=now + 61)
...
def test_max_records_per_client_bounds_memory():
    ...
    summary = telemetry.client_summary("10.0.0.5", now=now + 1)
...
def test_min_interval_detects_sub_second_loops():
    ...
    summary = telemetry.client_summary("10.0.0.5", now=base + 1)
...
def test_sources_breakdown_lists_callers():
    telemetry = RequestTelemetry(window_seconds=60, max_records_per_client=100)
    now = time.monotonic()
    _record(telemetry, "10.0.0.5", "/v1/a/", source="compA")
    _record(telemetry, "10.0.0.5", "/v1/a/", source="compA")
    _record(telemetry, "10.0.0.5", "/v1/b/", source="compB")
    summary = telemetry.client_summary("10.0.0.5", now=now)
...
def test_clients_are_isolated():
    telemetry = RequestTelemetry(window_seconds=60, max_records_per_client=100)
    now = time.monotonic()
    _record(telemetry, "10.0.0.5", "/v1/a/")
    _record(telemetry, "10.0.0.6", "/v1/b/")
    assert telemetry.client_summary("10.0.0.5", now=now)["total"] == 1
    assert telemetry.client_summary("10.0.0.6", now=now)["total"] == 1
...
def test_record_is_thread_safe():
    ...
    def worker(n: int) -> None:
        for i in range(200):
            _record(telemetry, f"10.0.0.{n}", f"/v1/p{i % 7}/")
    ...
    for t in threads:
        t.join()
    total = sum(
        telemetry.client_summary(f"10.0.0.{n}")["total"] for n in range(4)
    )
    assert total == 800
